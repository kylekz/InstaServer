...
public class NPCs extends SubBase {
	public NPCs(InstaServer pl) {
		super(pl);
	}

	@Override
	public void run(CommandSender sender, String[] args) {
		if (sender.hasPermission("instaserver.npcs")) {
			boolean enable = Boolean.parseBoolean(args[0]);
            plugin.getMinecraftServer().spawnNPCs = enable;
            plugin.getProps().put("spawn-npcs", String.valueOf(enable));
            try {
            	plugin.getProps().store(new FileOutputStream(plugin.getPropsPath()), null);
    			plugin.send(sender, plugin.info + "Set NPC spawning.");
    		} catch (IOException e) {
